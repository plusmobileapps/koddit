...
class Actions {
    ...
    data class PostDetailOpened(val post: RedditPostResponse)
...
fun rootReducer(state: AppState, action: Any): AppState {
    return when (action) {
        ...
        is Actions.PostDetailOpened -> state.copy(postDetail = action.post).also { Napier.d("${action.post}", null, "Post clickedd") }
