...
external interface VideoListProps: RProps {
    var posts: List<RedditPost>
}

class TestCards : RComponent<VideoListProps, RState>() {
    private var count = 0
    ...
    override fun RBuilder.render() {
        styledDiv {
            ...
            mGridContainer(MGridSpacing.spacing2) {
                ...
                props.posts.forEach { meme ->
                    mGridItem(breakpoints) {
                        mCard {

                            mCardHeader(title = meme.data.title) {}
                            mCardMedia(image = meme.data.url) {
                                css {
                                    height = 500.px
                                }
                            }
...
fun RBuilder.testCards(posts: List<RedditPost>) = child(TestCards::class) {
    attrs.posts = posts
}